...
citation:
  ...
  issue: 2025-24
  url: https://gallegre.github.io/GainEmploi/
annee: 2025
wp: 24
date: "2025-11-21"
date-modified: today
...
if(knitr::is_html_output()) 
  update_theme(text = element_text(size = 8)) else  
...
[^1]: @sicsic2023elas propose une revue de littérature sur l'élasticité de l'offre de travail.
...
[^2]: @devettervalentin2025IS estiment, à partir des déclarations annuelles de données sociales, que moins de 5% des salariés sont réminérés en dessous du smic horaire soit en raison de dérogations au droit commun dans certaines branches (assitantes maternelles ou représentants de commerce par exemple) soit en lien avec un décompte incomplet des heures de travail.
...
On peut définir le taux effectif de gain à l'emploi (TEG) comme la part de l'augmentation du revenu professionnel qui se traduit en augmentation du revenu disponible. Si lorsque le revenu d'activité augmente de 1000 euros, le revenu disponible augmente de 600 euros, on dira que le TEG est de 60%. $$
...
castyp <- readxl::read_xlsx("data/Sofi2024_cas_types_smic.xlsx", sheet = "TGE")
seuilpm=1312 # seuil de pauvreté à 60% actualisé en 2023. voir note dans le texte.
smic=1398.7 # SMIC mensuel net en 2023
...
labels_rev <- c("Salaire" = "Salaire", "PF" = "Prestations familiales", "PL" = "Prestations logement", 
               "RSA" = "RSA", "PA" = "Prime d'activité")
...
graph <- ggplot(basegr %>% filter(name !='RDISP'), aes(fill = name,
  ...
  theme(legend.position.inside =  c(0.05, 1),
        legend.justification = c(0,1),    
        plot.caption = element_marquee(margin = margin(t=9))
        ) +
  xlab(NULL) + ylab(NULL) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2500), breaks = seq(0, 2500, by = 500)) + 
  ofce_caption(
    source = "législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI"
    )
...
```{r}
#| label: fig-C2A2ECI
#| fig-cap: Revenu disponible d'un couple avec deux enfants selon le revenu professionnel, conjoint inactif
#| fig-asp: 0.7
...
```{r}
#| label: fig-garde
#| fig-cap: Revenu disponible d'une famille avec un enfant gardé par sa mère et/ou une assistante maternelle
#| fig-asp: 0.8


castyp <- readxl::read_xlsx("data/Sofi2024_cas_types_smic.xlsx", sheet = "TGE_JE")
...
base<-castyp %>% 
   mutate(typrev=case_when(
    ...
    TRUE ~ "Temps plein à 1,5 S"),
    typfam=case_when(isocou=="isolé" & nbenf==1 ~ "Parent isolé",
                           isocou=="couple" & actconj=="1 smic" & nbenf==1 ~ "En couple\nconjoint au smic",
                     TRUE ~ "autre"),
    CGNET=-CGNET
  ) %>% 
  filter(typfam!="autre") %>% 
  select(typrev,typfam,CGNET,Ract_conj,Salaire,PF,PL,RSA,PA, RDISP_NET_CG)	

basegr<-base %>% 
  pivot_longer(cols=c(CGNET,Ract_conj,Salaire,PF,PL,RSA,PA, RDISP_NET_CG)) 


basegr$typrev<-factor(basegr$typrev,levels=c("Sans revenu","Mi-temps au Smic","Temps plein au Smic","Temps plein à 1,5 S"))

basegr$name<-factor(basegr$name, levels=rev(c("CGNET","Ract_conj","Salaire", "PF", "PL", "RSA", "PA", "RDISP_NET_CG")))

basegr$typfam<-factor(basegr$typfam,levels=c("Parent isolé","En couple\nconjoint au smic"))


graph_cmg <- ggplot(basegr %>% filter(name !='RDISP_NET_CG'), aes(fill = name,
                                                                y = value, x = typrev)) + 
  facet_wrap(~ typfam)+
  scale_fill_manual(
   ...
   geom_bar_interactive(
    ...
    aes(
      x = typrev, y = round(value, 2), 
      colour = "Revenu disponible net de la garde", 
      tooltip=paste("Revenu disponible net de la garde :",round(value,0), "euros")), 
    size = 3
  ) +
    scale_color_manual(
    values = c("Revenu disponible net de la garde" = "orange")  
  ) +
  theme(legend.position.inside =  c(0.05, 1),
        legend.justification = c(0,1),
        panel.border = element_rect(colour = "grey", fill="transparent", linetype="solid", linewidth = 0.2),   
        plot.caption = element_marquee(margin = margin(t=9))
        ) +
  xlab(NULL) + ylab(NULL) +
    guides(
       colour=guide_legend(title = NULL, order = 1) ,
       fill = guide_legend(title = NULL, order = 2),
       x = guide_axis(n.dodge=2), color = guide_legend(title = NULL)
  )+
...
base<-castyp %>% 
  mutate(
    casconj=case_when(isocou=="isolé" ~ "Sans conjoint",
                  isocou=="couple" & actconj=="Inactif" ~ "Conjoint \nsans revenu",
                  isocou=="couple" & actconj=="Chômeur" ~ "Conjoint \nchômeur",
                  isocou=="couple" & actconj=="1 smic" ~ "Conjoint \nsalarié au smic",
                  TRUE ~ "Conjoint \nsalarié à 1,5 smic"),
  ...
  select(casconj,enf,Salaire,act,RDISP,RDISP_SSPA) 

basegr1<-base %>% 
  pivot_wider(names_from=act, values_from = c(Salaire, RDISP,RDISP_SSPA)) %>% 
  mutate(
    TGM=round(100*((RDISP_50-RDISP_0)/(Salaire_50-Salaire_0)),1),
    TGMspa=round(100*((RDISP_SSPA_50-RDISP_SSPA_0)/(Salaire_50-Salaire_0)),1),
    emploi="Emploi à mi-temps"
 ) %>% 
  select(casconj,enf,emploi,TGM,TGMspa)

basegr2<-base %>% 
  pivot_wider(names_from=act, values_from = c(Salaire, RDISP,RDISP_SSPA)) %>% 
  mutate(
    TGM=round(100*((RDISP_100-RDISP_0)/(Salaire_100-Salaire_0)),1),
    TGMspa=round(100*((RDISP_SSPA_100-RDISP_SSPA_0)/(Salaire_100-Salaire_0)),1),
    emploi="Emploi à temps plein"
 ) %>% 
  select(casconj,enf,emploi,TGM,TGMspa)

basegr<-rbind(basegr1, basegr2)

basegr$enf <- factor(basegr$enf, levels=c("0 enfant","1 enfant","2 enfants","3 enfants"))

basegr$casconj <- factor(basegr$casconj,levels=c("Sans conjoint","Conjoint \nsans revenu",
                                               "Conjoint \nchômeur","Conjoint \nsalarié au smic",
                                               "Conjoint \nsalarié à 1,5 smic"))

basegr$emploi <- factor(basegr$emploi, levels=c("Emploi à mi-temps","Emploi à temps plein"))


base_long <- basegr %>%
  select(enf, emploi, casconj, TGM,TGMspa) %>%
  pivot_longer(cols = c(TGM,TGMspa),
               names_to = "prime_activite",
               values_to = "TGM") %>%
  mutate(prime_activite = ifelse(prime_activite == "TGM", "Avec \nprime d'activité", "Sans \nprime d'acivité"))


graph4 <- ggplot() +
  geom_point_interactive(
    data = base_long, aes(x =casconj, y = TGM, 
      colour = enf, 
      tooltip=paste("Taux effectif de gain à l'emploi :",TGM, "%")), 
    size = 2
  )+
  facet_wrap(~emploi,nrow=1)+
  ...
  facet_grid(prime_activite ~ emploi)+
  guides(x = guide_axis(n.dodge=2), color = guide_legend(title = NULL),nrow=1) +
  ofce_caption(source ="législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI")
...
Le graphique montre que la prime d'activité a réduit les gains marginaux à l'emploi autour de 1 à 1,2 Smic. La courbe des gains marginaux suit maintenant une courbe en U et, à l'inverse, la courbe des taux marginaux effectifs de prélèvement, une courbe en U inversé, en contradiction avec la courbe "optimale" en U des taux marginaux, telle que décrite par @diamond1998 ou @saez2001. Selon ces auteurs, les taux marginaux effectifs de prélèvement les plus bas devraient s'appliquer au milieu de l'échelle de revenus, là où les individus sont les plus nombreux.

Avec les réformes visant à rendre le travail payant, les plus faibles gains à l'emploi se sont déplacés au niveau du Smic à temps complet ou juste au-dessus (@fig-gainsmarginaux). Certains pourraient dire que les "trappes à inactivité" se sont transformées en "trappes à bas salaires" mais ces expressions ne sont pas appropriées puisqu'elles impliquent que les travailleurs répondent à ces plus faibles incitations. Or, il est difficile de trouver des études confirmant ces effets sur l'offre de travail en pratique. Selon le rapport @boziowasmer2024 les politiques d'exonérations sociales (ciblées autour du Smic à temps-plein), si les effets théoriques des exonérations et des primes d'activité dégressives sont soulignés régulièrement par les économistes, "la démonstration empirique de l'existence de trappes à bas salaire est néanmoins délicate", la littérature disponible en France offrant des preuves empiriques limitées.
...
castyp <- readxl::read_xlsx("data/Sofi2024_cas_types_smic.xlsx", sheet = "TGME")

base<-castyp %>% 
  mutate(
    cas = case_when(isocou=="isolé" & nbenf==0 ~ "Isolé sans enfant",
                    isocou=="isolé" & nbenf==1 ~ "Isolé 1 enfant",
                    isocou=="isolé" & nbenf==2 ~ "Isolé 2 enfants",
                    isocou=="couple" & actconj=="Inactif" & nbenf==0 ~ "Couple monoactif sans enfant",
                    isocou=="couple" & actconj=="Inactif" & nbenf==1 ~ "Couple monoactif 1 enfant",
                    ...
                    isocou=="couple" & actconj=="1 smic" & nbenf==1 ~ "Couple biactif 1 enfant",
                    isocou=="couple" & actconj=="1 smic" & nbenf==2 ~ "Couple biactif deux enfants", 
                    TRUE ~ "autre"
  )) %>% 
  filter(cas != "autre")  %>% 
  mutate(
    gainmarginal = round(RDISP,1),
    gainmarginal_sspa = round(RDISP-PA,1)
...
base$cas <- factor(base$cas, 
                   levels = c("Isolé sans enfant","Isolé 1 enfant","Isolé 2 enfants", 
                              "Couple monoactif sans enfant","Couple monoactif 1 enfant","Couple monoactif deux enfants", 
                              "Couple biactif sans enfant","Couple biactif 1 enfant", "Couple biactif deux enfants"))
...
graphG2<-ggplot() +
  geom_col_interactive(data = base_long, 
                       aes(x = revenu, y = valeur, fill = variable,
           tooltip=paste("Gain marginal pour 100 € de salaire en plus:",valeur, "euros")), 
           position = position_dodge(width = 0.6),  
           width = 0.4) + 
  facet_wrap(~ cas) +
  scale_fill_manual(values = c("Sans prime d'activité" = "#482677FF", "Avec prime d'activité" = "#95D840FF")) +
  scale_y_continuous(limits = c(-50, 100),breaks = seq(-50, 100, by = 25)) +
  ...
  theme(
    ...
    xlab("Revenu individuel en parts de smic")+ylab(NULL) +
  ofce_caption(source ="législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI")
...
Si l'emploi est toujours rémunérateur, il ne protège pas toujours de la pauvreté monétaire (voir @tip-tpm). Ceci apparaît clairement sur cas-types mais peut se voir également par l'existence d'une pauvreté monétaire en emploi, même pour des bénéficiaires de la prime d'activité[^3]. En 2022, selon l'INSEE le taux de pauvreté sur l'ensemble de la population était de 14,4%. et celui des personnes en emploi de 7,7 %. Mais en se limitant aux personnes en emploi vivant dans un ménage allocataire de la prime d'activité, ce taux atteint 15,2 % d'après nos estimations à partir de l'Enquête revenus fiscaux et sociaux de 2022. L'emploi réduit ainsi beaucoup le risque de pauvreté monétaire mais ne l'annule pas : sur les 9,1 millions de personnes vivant sous le seuil de pauvreté monétaire, 2 millions sont en emploi (INSEE).
...
::: {#tip-tpm .callout-tip collapse="false" icon="false"}
...
Le niveau de vie d'un célibataire sans enfant sans revenus d'activité et recourant à toutes les prestations est de 851 euros par UC, ce qui est inférieur de 35% au seuil de pauvreté monétaire : son intensité de pauvreté (c'est-à-dire l'écart au seuil de pauvreté) est donc de 35% ( @fig-nivvie). Un emploi à mi-temps au Smic ne lui permet pas de franchir le seuil de pauvreté (intensité de 9%). Le célibataire sans enfant franchit le seuil de pauvreté monétaire avec un Smic à plein temps même s'il ne recourt pas à la prime d'activité et son niveau de vie atteint 126% du seuil s'il y recourt. En revanche, un emploi à mi-temps ne permet pas de sortir de la pauvreté même si la prime d'activité en réduit l'intensité (de 30% avant prime à 9% après).
...
base<-castyp %>% 
  mutate(
    cas = case_when(isocou=="isolé" ~ "Isolé",
                    ...
                    isocou=="couple" & actconj=="1 smic" ~ "conjoint au Smic",
                    isocou=="couple" & actconj=="Chômeur" ~ "conjoint au chômage", 
                    TRUE ~ "autre"),
  emp=case_when(
    Salaire==0 ~ "Sans emploi",
    Salaire==699.35 ~ "Emploi à mi-temps au Smic",
  ...
  filter(cas != "autre" & nbenf<3 & emp !="autre") %>% 
  select(cas, nbenf, NVIE,NVIESSPA,emp)

base$emp <- factor(base$emp, levels = c("Sans emploi", "Emploi à mi-temps au Smic", "Emploi à temps plein au Smic"))

base$cas <- factor(base$cas, levels = c("Isolé", "conjoint sans revenu", "conjoint au Smic","conjoint au chômage"))

base$nbenf <- factor(base$nbenf, levels = c("0", "1", "2")) 

seuils <- data.frame(
  seuil = c(seuilpm, seuilpm*4/6),
  type = c("Seuil de pauvreté monétaire (60 %)", "Seuil de grande pauvreté monétaire (40 %)")
)

base_long <- base %>%
  select(nbenf, emp, cas, NVIE, NVIESSPA) %>%
...
graph_pm<-ggplot() +
    geom_point_interactive(
    data = base_long, aes(x =nbenf, y = round(montant,0), 
      colour = emp, 
      tooltip=paste("Niveau de vie :",round(montant,0), "€")), 
    size = 1.5
  )+
  ...
  scale_linetype_manual(
    name = "", 
    values = c("Seuil de pauvreté monétaire (60 %)" = "solid", 
               "Seuil de grande pauvreté monétaire (40 %)" = "dashed")
  ) +
  labs(color = "Statut d'emploi",
       linetype = "Seuils") +
  facet_grid(prime_activite ~ cas)+
  guides(x = guide_axis(n.dodge=1), color = guide_legend(title = NULL)) +
  theme(
    legend.position = "bottom",
    plot.caption = element_marquee(margin = margin(t=9)),
    panel.border = element_rect(colour = "grey", fill="transparent", linetype="solid", linewidth = 0.2),
    ) +
  xlab("Nombre d'enfants")+ylab(NULL)+
  ofce_caption(source ="législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI")
...
srcv<-read_excel("data/Sorties_Privation.xlsx",sheet = "Sorties_Privation")
...
pivot_longer(cols = c(TX_PAUVRETE, TX_ISO_PAUVRETE, TX_COU_PAUVRETE), names_to = "cas", values_to = "taux") %>%
  ...
  mutate(mesure="Pauvreté monétaire",
         cas = fct_recode(cas,
...
pivot_longer(cols = c(TX_PMS, TX_ISO_PMS, TX_COU_PMS), names_to = "cas", values_to = "taux") %>%
  select(modalite, cas, taux) %>%
  filter(modalite!="Ensemble")%>% 
  mutate(mesure="Privations matérielles et sociales",
         cas = fct_recode(cas,
                              "Ensemble" = "TX_PMS",
...
base$modalite<-factor(base$modalite, levels=c("< 0,3 smic", "[0,3 à 0,6[", "[0,6 à 0,9[", "[0,9 à 1,2[", "[1,2 à 1,5[", "[1,5 à 1,8[", "1,8 et +"))


graphG <- ggplot() +
  geom_col_interactive(data = base, aes(x = modalite, y = taux, fill = cas,
                             tooltip=paste("Taux=",round(taux,1), "%")), 
           position = position_dodge(width = 0.6),  # Position des barres
           ) +  # Largeur des barres
  facet_wrap(~mesure)+
  scale_y_continuous(limits = c(0, 51)) +
  labs(x = "Revenu en tranche de Smic",
       y = "Taux en %") + 
  theme(
    legend.position = "bottom",
    plot.caption = element_marquee(margin = margin(t=9)),
  ...
  guides(x = guide_axis(n.dodge=2), color = guide_legend(title = NULL)) +
  ofce_caption(source ="législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI")  
...
Dans les cas-types étudiés, les revenus d'assistance procurent un niveau de vie inférieur au seuil de pauvreté à 60 %, et même inférieur au seuil de grande pauvreté à 40 % pour les couples et les personnes seules ( @fig-ndvminimum).
...
graph5 <- ggplot(base, aes(x = isocou, y = NVIE, fill = enf)) + 
  geom_bar_interactive(stat = "identity", position = position_dodge(width = 0.6),  
           width = 0.4,
    aes(y=NVIE, fill=enf,tooltip=paste("niveau de vie :",round(NVIE,0), "euros"))
  ) + 
  scale_y_continuous(limits = c(0, 1500)) +  
    geom_hline(
  aes(yintercept = (4/6)*seuilpm, linetype = "Seuil de grande pauvreté monétaire (40 %)"),
              color = "grey40",linewidth = 0.5)+
    geom_hline(aes(yintercept = seuilpm,linetype = "Seuil de pauvreté monétaire (60 %)"),
    color = "grey40",linewidth = 0.5)+
  scale_linetype_manual(name = "Références", values = c("Seuil de pauvreté monétaire (60 %)" = "solid", "Seuil de grande pauvreté monétaire (40 %)" = "dashed"),
                        limits = c("Seuil de pauvreté monétaire (60 %)", "Seuil de grande pauvreté monétaire (40 %)"))+
    guides(
...
```{r}
#| label: fig-minima
#| fig-cap: Evolution des mimina sociaux et du Smic horaire, base 100 en 1990
#| fig-asp: 0.8


Bareme <- read_excel("data/DREES-BaremesMS.xlsx")
...
[^4]: Le niveau requis est estimé pour la personne seule allocataire des aides au logement en zone 2 en zone 2 (villes de plus de 100 000 habitants).

Cette réforme est celle qui permettrait d'atteindre simultanément les trois objectifs de manière cohérente. Elle est également la plus juste dans la mesure où elle réduirait fortement les inégalités avec, probablement, de très faibles coûts en termes d'efficacité et d'emploi (voir par exemple @bargainvicard2014 et @simmonetdanzin2014). Mais elle est aussi la plus coûteuse. Comme le montre notre simulation (voir @tip-simul), augmenter le socle à 900 euros avec une légère baisse du taux de cumul de la prime d'activité aurait un coût d'environ 24 milliards d'euros pour les pouvoirs publics, dont 11 milliards de RSA et 13 milliards de prime d'activité. Le dilemme entre redistribution et incitation est en fait un triangle d'incompatibilité entre redistribution, incitations monétaires et coût budgétaire @boziogrenet2017).
...
::: {#tip-simul .callout-tip collapse="false" icon="false"}
